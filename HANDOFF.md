...
- 階段：**W0–W3 ✅ ＋ 工程強化 ✅ ＋ RAG 優化 ①②③ ✅ ＋ 結構化 DB 層 ✅ ＋ pgvector ✅ ＋ 多輪對話記憶 ✅ ＋ 數值幻覺護欄 ✅ ＋ E3 打包 ✅ ＋ CI gate ✅ ＋ 全專案稽核 43 確認問題全修 ✅ ＋ git/GitHub 上線（JerryHuang0829/CiteRAG）＋ 雲端 LLM router ✅（`CITERAG_LLM_BACKEND=cloud`：Gemini Flash-Lite 主 + Groq fallback；離線測試 63 passed）**。
  ...
  ＋ **加分 #1-#5 ✅**：多模態（雲端 VLM=Gemini vision）· golden 擴台泥+鴻海（32 題 recall 1.000）· 元件 ablation（rerank 是關鍵/hybrid 誠實負結果）· **Grafana 可跑 observability stack**（`observability/`，實測 provision 成功）· **gate 抓 regression 實證**（關 rerank→recall 0.812<SLO→exit1）。#6 P4 IaC 略過（Lane 2 零損失）。
  下一步（**建置全完成、剩出貨**）：① 你 **GitHub Settings→Secrets 加 key** ② **push** `origin`(--force；LFS ~28MB, 13 commits)+`space` ③ **投履歷**（case_study §9 bullet + demo URL + repo → 玉山/中信/永豐/Appier）。技術面已無缺口。
